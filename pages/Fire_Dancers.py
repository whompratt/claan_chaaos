...
st.set_page_config(page_title=CLAAN, page_icon=":fire:", layout="wide")
...
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["password"] == st.secrets.passwords.get("fire_dancers"):
            st.session_state["fire_password_correct"] = True
            del st.session_state["password"]  # don't store password
        else:
            st.session_state["fire_password_correct"] = False

    if "fire_password_correct" not in st.session_state:
    ...
    elif not st.session_state["fire_password_correct"]:
...
if "db" not in st.session_state:
    client: MongoClient = MongoClient(**st.secrets["mongo"])
    ...
    st.session_state.col = st.session_state.db["fire_dancers"]
...
def get_data():
    st.session_state.total_score = [
        i["Score"] for i in st.session_state.score_col.find({"Claan": CLAAN})
    ]
    scores = [
        i
        for i in st.session_state.score_col.find(
            {
                "Date": {
                    "$gte": datetime.datetime.strptime(
                        st.session_state.settings["Fortnight Start Date"], ("%d/%m/%Y")
                    )
                },
                "Claan": CLAAN,
            }
    ...
    st.session_state.submissions = [
        i
        for i in st.session_state.col.find(
            {
                "Date": {
                    "$gte": datetime.datetime.strptime(
                        st.session_state.settings["Fortnight Start Date"], ("%d/%m/%Y")
...
def submit_quest():
    if st.session_state.qname == "Please select your name":
        ...
        roll = random.randint(
            1, st.session_state.settings["Quests"][st.session_state.quest]
        )
...
def submit_activity():
    if st.session_state.aname == "Please select your name":
        ...
        roll = random.randint(
            1, st.session_state.settings["Activities"][st.session_state.activity]
        )
        ...
        submission = {
            ...
            "Dice step": st.session_state.settings["Activities"][
    ...
    if check_password():
        ...
        with st.container():
            ...
            with head_l:
                ...
                st.title(CLAAN)
                st.write(
                    f"Welcome to the {CLAAN} Claan area! Here you can log quests, activities and steps!"
                )
                ...
                with col4:
                    st.metric(
                        "Activities Completed", st.session_state.fortnight_activities
                    )
            with head_r:
                # Add logo
                st.image(claan_img)
        ...
        with st.container():
            ...
            with log_l:
                ...
                # Name
                st.session_state.qname = st.selectbox(
                    "Please tell me who you are!",
                    tuple(
                        ["Please select your name"] + st.session_state.settings[CLAAN]
                    ),
                    key=0,
                )

                # Acitivity/challenge
                st.session_state.quest = st.radio(
                    "What quest did you complete?", st.session_state.settings["Quests"]
                )
            ...
            with log_r:
                ...
                # Name
                st.session_state.aname = st.selectbox(
                    "Please tell me who you are!",
                    tuple(
                        ["Please select your name"] + st.session_state.settings[CLAAN]
                    ),
        ...
        with st.container():
            ...
            st.write(
                "Press the button below to see the details of what people have been up to! (It is hidden by default as it can be a little slow to load!)"
            )
