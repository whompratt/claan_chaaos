...
def main():
    ...
    with st.container():
        ...
        # Add description
        st.write(
            "Welcome to seasion 4 of Claans at Advancing Analytics. This time around things are taking a more relaxed turn but retaining a healthy dose of that Claans flair!"
        )
        st.write(
            "Using the Claan Portal you can see the scores as they stand, see this fortnights quests, and access the Claan area log quests, steps and activities!"
        )
    ...
    with st.container():
        ...
        with col1:
            ...
            earth_scores = [
                i["Score"] for i in scores if (i["Claan"] == "Earth Striders")
            ]
            print(earth_scores)
            # Add metric for claan score
            st.metric(
                label="Earth Striders", value=sum(earth_scores), delta=earth_scores[-1]
            )
        ...
        with col2:
            ...
            st.metric(
                label="Fire Dancers", value=sum(fire_scores), delta=fire_scores[-1]
            )
        ...
        with col3:
            ...
            # Get scores for the claan
            thunder_scores = [
                i["Score"] for i in scores if (i["Claan"] == "Thunder Walkers")
            ]
            # Add metric for claan score, using the sum of the scores, and the last entry as the delta
            st.metric(
                label="Thunder Walkers",
                value=sum(thunder_scores),
                delta=thunder_scores[-1],
            )
        ...
        with col4:
            ...
            # Add metric for claan score, using the sum of the scores, and the last entry as the delta
            st.metric(
                label="Wave Riders", value=sum(wave_scores), delta=wave_scores[-1]
            )
